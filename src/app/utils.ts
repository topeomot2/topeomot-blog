...
export async function getPostDetails(postId: string): Promise<Post> {
    try {
        ...
        const fileContent = await fs.readFile(`${directoryPath}/${postId}.mdx`, {
            encoding: "utf-8",
        });
...
export async function getFilesByExtension(
    ...
    return posts
        .filter((post) => post.content && post.meta)
        .sort((post1, post2) => Date.parse(post2.meta!.date!) - Date.parse(post1.meta!.date!))

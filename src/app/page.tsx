...
export default async function Page() {
  ...
  return <>
    ...
    {files.map((file, index) => {
      return (<div key={index} className="w-full border border-slate-500 rounded mb-5 p-5">
        <Link href={`posts/${file.postId}`} className="hover:underline text-base font-bold text-slate-700 dark:text-slate-200">{file.meta.title}</Link>
        <div className="text-sm">{file.meta.description}</div>
        <div>{file.meta.date && format(new Date(Date.parse(file.meta.date)), "PPP")}</div>
      </div>)
    })}
